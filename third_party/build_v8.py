...
def v8deps():
    ...
    subprocess.check_call(["gclient", "sync", "--spec", spec], cwd=deps_path, env=env)
...
def main():
    ...
    subprocess.check_call(
        [gn_path, "gen", build_path, "--args=" + gen_args], cwd=v8_path, env=env
    )
    subprocess.check_call(
        [ninja_path, "-v", "-C", build_path, "v8_monolith"], cwd=v8_path, env=env
    )
